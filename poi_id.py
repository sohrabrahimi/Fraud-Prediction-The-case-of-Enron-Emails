...
# After the feature selection procedure, it is important to tune the learning algorithm to obtain the best performance. Tuning can substantially influence the performance of the algorithm by primarily avoiding overfitting or creating models with high bias. For example, in case of the random forest algorithm, one cans et the number ot trees used in the algorith, having too many trees leads to overfitting and too few models would not give us the accuracy that we desire from a model. Same thing can be said about SVM, should we use linear or rbf kernels? which one perform better for our dataset? It is therefore important to efficiently try a variety of parameters and eventually select the best combination.
# In[218]:
